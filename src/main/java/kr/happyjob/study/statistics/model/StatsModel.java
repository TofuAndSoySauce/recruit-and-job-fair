package kr.happyjob.study.statistics.model;

public class StatsModel {

	private int notice_no;
	private int file_cd;
	private int is_delete;
	private String update_date;
	private String reg_date;
	private String notice_content;
	private String notice_title;
	private int notice_count;
	private String writer;
	private String file_name;
	private String file_nadd;
	private String file_madd;
	private int file_size;
	// --------------------------------
	private String loginID;
	private String name;
	private String age;
	private String sex;
	private String position;
	private String location;
	// col ----------------------------------
	private String ent;
	private String pct;
	// bar ---------------------------
	private String ent2;
	private String age_c;
	private int age_cnt;
	private String sex_c;
	// bar2 ------------------------
	private String ent2_2;
	private String age_c2;
	private int age_cnt2;
	private String sex_c2;
	// fair ----------------------------
	private int fno;
	private String fttl;
	private String floc;
	private String fst;
	private String fed;
	private String facn;
	private int faun;
	// fair pie 1 ---------------------------
	private int fno2;
	private String sex_fc;
	private int sex_fcnt;
	// fair pie 1_2 -------------------------
	private int fno2_2;
	private String sex_fc2;
	private int sex_fcnt2;
	// fair pie 2 ---------------------------
	private int fno3;
	private String c_size;
	private int c_cnt;
	// fair pie 2_2 ---------------------------
	private int fno3_2;
	private String c_size2;
	private int c_cnt2;
	// ---------------------------------------------
	// ---------------------------------------------
	public int getFno3_2() {
		return fno3_2;
	}

	public void setFno3_2(int fno3_2) {
		this.fno3_2 = fno3_2;
	}

	public String getC_size2() {
		return c_size2;
	}

	public void setC_size2(String c_size2) {
		this.c_size2 = c_size2;
	}

	public int getC_cnt2() {
		return c_cnt2;
	}

	public void setC_cnt2(int c_cnt2) {
		this.c_cnt2 = c_cnt2;
	}
	
	public int getFno2_2() {
		return fno2_2;
	}

	public void setFno2_2(int fno2_2) {
		this.fno2_2 = fno2_2;
	}

	public String getSex_fc2() {
		return sex_fc2;
	}

	public void setSex_fc2(String sex_fc2) {
		this.sex_fc2 = sex_fc2;
	}

	public int getSex_fcnt2() {
		return sex_fcnt2;
	}

	public void setSex_fcnt2(int sex_fcnt2) {
		this.sex_fcnt2 = sex_fcnt2;
	}
	
	public String getEnt2_2() {
		return ent2_2;
	}

	public void setEnt2_2(String ent2_2) {
		this.ent2_2 = ent2_2;
	}

	public String getAge_c2() {
		return age_c2;
	}

	public void setAge_c2(String age_c2) {
		this.age_c2 = age_c2;
	}

	public int getAge_cnt2() {
		return age_cnt2;
	}

	public void setAge_cnt2(int age_cnt2) {
		this.age_cnt2 = age_cnt2;
	}

	public String getSex_c2() {
		return sex_c2;
	}

	public void setSex_c2(String sex_c2) {
		this.sex_c2 = sex_c2;
	}
	
	public int getFno2() {
		return fno2;
	}

	public void setFno2(int fno2) {
		this.fno2 = fno2;
	}

	public String getSex_fc() {
		return sex_fc;
	}

	public void setSex_fc(String sex_fc) {
		this.sex_fc = sex_fc;
	}

	public int getSex_fcnt() {
		return sex_fcnt;
	}

	public void setSex_fcnt(int sex_fcnt) {
		this.sex_fcnt = sex_fcnt;
	}

	public int getFno3() {
		return fno3;
	}

	public void setFno3(int fno3) {
		this.fno3 = fno3;
	}

	public String getC_size() {
		return c_size;
	}

	public void setC_size(String c_size) {
		this.c_size = c_size;
	}

	public int getC_cnt() {
		return c_cnt;
	}

	public void setC_cnt(int c_cnt) {
		this.c_cnt = c_cnt;
	}
	
	public int getFno() {
		return fno;
	}

	public void setFno(int fno) {
		this.fno = fno;
	}

	public String getFttl() {
		return fttl;
	}

	public void setFttl(String fttl) {
		this.fttl = fttl;
	}

	public String getFloc() {
		return floc;
	}

	public void setFloc(String floc) {
		this.floc = floc;
	}

	public String getFst() {
		return fst;
	}

	public void setFst(String fst) {
		this.fst = fst;
	}

	public String getFed() {
		return fed;
	}

	public void setFed(String fed) {
		this.fed = fed;
	}

	public String getFacn() {
		return facn;
	}

	public void setFacn(String facn) {
		this.facn = facn;
	}

	public int getFaun() {
		return faun;
	}

	public void setFaun(int faun) {
		this.faun = faun;
	}
	
	//============================
	public String getEnt2() {
		return ent2;
	}

	public void setEnt2(String ent2) {
		this.ent2 = ent2;
	}

	public String getAge_c() {
		return age_c;
	}

	public void setAge_c(String age_c) {
		this.age_c = age_c;
	}

	public int getAge_cnt() {
		return age_cnt;
	}

	public void setAge_cnt(int age_cnt) {
		this.age_cnt = age_cnt;
	}

	public String getSex_c() {
		return sex_c;
	}

	public void setSex_c(String sex_c) {
		this.sex_c = sex_c;
	}
	
	public String getEnt() {
		return ent;
	}

	public void setEnt(String ent) {
		this.ent = ent;
	}

	public String getPct() {
		return pct;
	}

	public void setPct(String pct) {
		this.pct = pct;
	}
	
	public String getAge() {
		return age;
	}

	public void setAge(String age) {
		this.age = age;
	}

	public String getSex() {
		return sex;
	}

	public void setSex(String sex) {
		this.sex = sex;
	}

	public String getPosition() {
		return position;
	}

	public void setPosition(String position) {
		this.position = position;
	}

	public String getLocation() {
		return location;
	}

	public void setLocation(String location) {
		this.location = location;
	}
	
	public String getFile_name() {
		return file_name;
	}

	public void setFile_name(String file_name) {
		this.file_name = file_name;
	}

	public String getFile_nadd() {
		return file_nadd;
	}

	public void setFile_nadd(String file_nadd) {
		this.file_nadd = file_nadd;
	}

	public String getFile_madd() {
		return file_madd;
	}

	public void setFile_madd(String file_madd) {
		this.file_madd = file_madd;
	}

	public int getFile_size() {
		return file_size;
	}

	public void setFile_size(int file_size) {
		this.file_size = file_size;
	}

	public String getWriter() {
		return writer;
	}

	public void setWriter(String writer) {
		this.writer = writer;
	}

	public int getNotice_no() {
		return notice_no;
	}

	public void setNotice_no(int notice_no) {
		this.notice_no = notice_no;
	}

	public String getLoginID() {
		return loginID;
	}

	public void setLoginID(String loginID) {
		this.loginID = loginID;
	}

	public int getFile_cd() {
		return file_cd;
	}

	public void setFile_cd(int file_cd) {
		this.file_cd = file_cd;
	}

	public int getIs_delete() {
		return is_delete;
	}

	public void setIs_delete(int is_delete) {
		this.is_delete = is_delete;
	}

	public String getUpdate_date() {
		return update_date;
	}

	public void setUpdate_date(String update_date) {
		this.update_date = update_date;
	}

	public String getReg_date() {
		return reg_date;
	}

	public void setReg_date(String reg_date) {
		this.reg_date = reg_date;
	}

	public String getNotice_content() {
		return notice_content;
	}

	public void setNotice_content(String notice_content) {
		this.notice_content = notice_content;
	}

	public String getNotice_title() {
		return notice_title;
	}

	public void setNotice_title(String notice_title) {
		this.notice_title = notice_title;
	}

	public int getNotice_count() {
		return notice_count;
	}

	public void setNotice_count(int notice_count) {
		this.notice_count = notice_count;
	}

	public String getName() {
		return name;
	}

	public void setName(String name) {
		this.name = name;
	}
	// =============================================
	private String loginID2;
	private String name2;
	private String adtype;
	private String adexp;
	private String adedu;
	private String adsal;
	private String regdat;
	private String addat;
	private String adstat;
	private int is_delete2;
	// --------------------------
	private String loginID_input;
	// ============================
	public String getLoginID_input() {
		return loginID_input;
	}

	public void setLoginID_input(String loginID_input) {
		this.loginID_input = loginID_input;
	}

	public String getLoginID2() {
		return loginID2;
	}

	public void setLoginID2(String loginID2) {
		this.loginID2 = loginID2;
	}
	public String getName2() {
		return name2;
	}

	public void setName2(String name2) {
		this.name2 = name2;
	}
	public String getAdtype() {
		return adtype;
	}

	public void setAdtype(String adtype) {
		this.adtype = adtype;
	}

	public String getAdexp() {
		return adexp;
	}

	public void setAdexp(String adexp) {
		this.adexp = adexp;
	}

	public String getAdedu() {
		return adedu;
	}

	public void setAdedu(String adedu) {
		this.adedu = adedu;
	}

	public String getAdsal() {
		return adsal;
	}

	public void setAdsal(String adsal) {
		this.adsal = adsal;
	}

	public String getRegdat() {
		return regdat;
	}

	public void setRegdat(String regdat) {
		this.regdat = regdat;
	}

	public String getAddat() {
		return addat;
	}

	public void setAddat(String addat) {
		this.addat = addat;
	}

	public String getAdstat() {
		return adstat;
	}

	public void setAdstat(String adstat) {
		this.adstat = adstat;
	}
	
	public int getIs_delete2() {
		return is_delete2;
	}

	public void setIs_delete2(int is_delete2) {
		this.is_delete2 = is_delete2;
	}

}
