...
<script type="text/javascript">
	...
	function validation() {
		var community_titlefile = $("#community_titlefile").val();
		var community_detfile = $("#community_detfile").val();
		
		 
		if(community_titlefile == ""){
			swal("제목을 입력해주세요.").then(function() {
				$("#community_titlefile").focus();
			  });
			return false;
		}
		
		if(community_detfile == ""){
			swal("내용을 입력해주세요.").then(function() {
				$("#community_detfile").focus();
			  });
			return false;
		}
		return true;
	...
	<form id="cmtForm" action="" method="">
		...
		<div>
			<div style="margin-bottom: 10px;">
				<a href="" class="btnType blue" id="btnCommentsave" name="btn"
					style="float: right; margin-left: 5px; margin-right: 5px; padding-top: 7px; padding-right: 7px; color: white; height: 30px; ">댓글등록</a>
			</div>
		</div>
