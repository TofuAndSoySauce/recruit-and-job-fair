...
<c:forEach var="adModel" items="${adList }">
	...
	<tr>
		...
		<td>
			<c:choose>
				<c:when test="${adModel.ad_yn == '1' }">
					대기
				</c:when>
				<c:when test="${adModel.ad_yn == '2' }">
					반려
				</c:when>
				<c:when test="${adModel.ad_yn == '3' }">
					<c:choose>
						<c:when test="${now gt adModel.ad_date }">기간만료</c:when>
						<c:when test="${adModel.ad_status == '1' }">공고중</c:when>
						<c:when test="${adModel.ad_status == '2' }">공고취소</c:when>
						<c:when test="${adModel.ad_status == '3' }">모집완료</c:when>
					</c:choose>
				</c:when>
			</c:choose>
		</td>
